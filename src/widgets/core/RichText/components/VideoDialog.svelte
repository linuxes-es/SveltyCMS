	...
	>
		...
		{#if insert_url}
			<form onsubmit={handleSubmit} class="relative mt-2 flex flex-col items-center justify-center gap-4">
				...
				<button type="submit" class="preset-filled-primary btn w-full">Add Video</button>
			</form>
		{:else}
			<div class="relative mt-2 flex flex-col items-center justify-center gap-4">
				...
				<div class="flex w-full justify-center gap-2">
					<button class="preset-outline-primary btn w-full" disabled>Browse locally</button>
					<button class="preset-filled-secondary btn w-full" onclick={() => (insert_url = true)}> YouTube </button>
				</div>
			</div>
		{/if}
	</div>
