...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		const mediaResult = await db.crud.findOne<MediaItem>('MediaItem', { _id: mediaId as any });
		...
		const updateResult = await db.crud.update<MediaItem>('MediaItem', mediaItem._id, {
			metadata: {
				...mediaItem.metadata,
				tags: updatedTags
			}
