...
- ✗ `EditorTopToolbar.svelte` - Generic top controls
- ✗ `CropTopToolbar.svelte` - Crop-specific top controls
- ✗ `CropBottomBar.svelte` - Crop-specific bottom controls
- ✗ `FineTuneTopToolbar.svelte` - FineTune top controls
- ✗ `FineTuneBottomBar.svelte` - FineTune bottom controls
- ✗ `BlurTopToolbar.svelte` - Blur top controls
- ✗ `WatermarkTopToolbar.svelte` - Watermark top controls
- ✗ `AnnotateTopToolbar.svelte` - Annotate top controls
...
- ✓ `EditorToolbar.svelte` - ONE intelligent bottom toolbar
...
├── ImageEditor.svelte          # Main component (modal-ready)
├── EditorSidebar.svelte        # Tool selection sidebar
├── EditorToolbar.svelte        # ✨ NEW: Single adaptive bottom toolbar
├── EditorCanvas.svelte         # Canvas wrapper (keep as is)
│
...
## 🔧 EditorToolbar.svelte (New Single Toolbar)
...
<div class="fixed bottom-0 left-0 right-0 border-t bg-surface-100 p-4 dark:bg-surface-800">
	<div class="mx-auto flex max-w-7xl items-center justify-between gap-4">
		...
		<div class="flex items-center gap-2">
			...
			<button onclick={save} class="preset-filled-primary btn">Save</button>
		</div>
	</div>
</div>
```
...
## 🎛️ Tool-Specific Control Components
...
<div class="flex items-center gap-2">
	...
	<!-- Apply -->
	<button class="preset-filled-success btn" onclick={() => cropToolRef.apply()}> Apply Crop </button>
</div>
```
...
{#if isMobile}
	<div class="fixed bottom-0 left-0 right-0 space-y-3 bg-surface-100 p-3">
		...
		<div class="flex gap-2">
			...
			<button class="preset-filled-primary btn flex-1">Save</button>
		</div>
	</div>
{/if}
```
...
1. ✅ Create `EditorToolbar.svelte` (single bottom toolbar)
2. ✅ Create `toolbars/CropControls.svelte`
3. ✅ Create `toolbars/FineTuneControls.svelte`
4. ✅ Create `toolbars/BlurControls.svelte`
5. ✅ Create `toolbars/AnnotateControls.svelte`
6. ✅ Create `toolbars/WatermarkControls.svelte`

### Phase 2: Update Main Editor (Week 1)

7. ✅ Remove all top toolbar logic from `ImageEditor.svelte`
8. ✅ Replace with single `<EditorToolbar />` at bottom
9. ✅ Update layout to full-screen canvas
10. ✅ Test responsive behavior (desktop/mobile)

### Phase 3: Delete Old Components (Week 1)

11. ✅ Delete `CropTopToolbar.svelte`
12. ✅ Delete `CropBottomBar.svelte`
13. ✅ Delete `FineTuneTopToolbar.svelte`
14. ✅ Delete `FineTuneBottomBar.svelte`
15. ✅ Delete `BlurTopToolbar.svelte`
16. ✅ Delete `WatermarkTopToolbar.svelte`
17. ✅ Delete `AnnotateTopToolbar.svelte`
18. ✅ Delete `MobileToolbar.svelte`

### Phase 4: Polish & Test (Week 2)

19. ✅ 100% Tailwind CSS (remove custom classes)
20. ✅ Fix all TypeScript errors
21. ✅ Add smooth transitions between tools
22. ✅ Test all tools thoroughly
...
```
┌─────────────────────────────────────────┐
│  [Upload] [Filename]  [Undo][Redo][Save]│  ← Generic Top Toolbar
├─────────────────────────────────────────┤
│  [Rotate][Flip][Aspect][Shape][Done]   │  ← Crop Top Toolbar
├─────────────────────────────────────────┤
│                                         │
│         CANVAS (Cramped)                │
│                                         │
├─────────────────────────────────────────┤
│  [Rotation Slider] [Scale Slider]      │  ← Crop Bottom Bar
└─────────────────────────────────────────┘
```
...
│                                         │
├─────────────────────────────────────────┤
│ [Aspect][Shape][Rotate][Flip][Apply]   │  ← Single Bottom Toolbar
│                   [Undo][Redo][Save]    │     (Context-Aware)
└─────────────────────────────────────────┘
```
...
- MediaUpload: After selecting or uploading, “Edit” opens the editor modal with the image; on Apply, the client posts to `/api/media/process`, then updates the widget with the processed asset; focal point is saved via metadata PATCH.
- MediaGallery: “Edit” action opens the same modal with the selected image; follows the same flow as above.
