...
description: 'Complete guide to the SveltyCMS Image Editor, its Pintura-style redesign, non-destructive workflow, and architecture.'
order: 15
icon: 'mdi:image-edit'
author: 'admin'
created: '2025-12-03'
...
The SveltyCMS Image Editor is a powerful, integrated tool for media manipulation, redesigned with a modern, "Pintura-style" user experience and a non-destructive workflow. This guide covers its architecture, features, and how it integrates into the CMS.

## 🏗️ Architecture & UI: The "Pintura-Style" Experience
...
-   **Crop:** Crop, rotate, and flip.
-   **Blur:** Apply regional blur or pixelation. The bug that caused this to break the undo/redo stack has been **fixed**.
-   **FineTune:** Adjust brightness, contrast, saturation, and more.
-   **Annotate:** Add text, arrows, and shapes to the image.
...
To enable intelligent, art-directed cropping for responsive images, you can now set a focal point.

-   **In the `MediaUpload` Widget:** When you upload an image, a **`+`** crosshair icon appears over the preview.
-   **Set the Point:** Drag this icon to the most important part of the image (e.g., a person's face, a product detail).
-   **Save:** The focal point coordinates (as percentages) are saved to the image's metadata.
-   **Usage:** Your frontend code can use these coordinates in conjunction with CSS (`object-position`) or an image transformation service to ensure the most important part of the image is always visible, no matter the aspect ratio.
...
-   **[Image Editor Architecture](/docs/architecture/image-editor-architecture.mdx):** A technical deep-dive into the editor's components.
